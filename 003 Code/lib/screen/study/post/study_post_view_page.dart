...
import 'package:circlet/screen/study/edit/study_post_edit_page.dart';
...
class StudyPostViewPage extends StatefulWidget {
  @override
  State<StudyPostViewPage> createState() => _StudyPostViewPageState();
  final Map<String, dynamic> postInfo;

  StudyPostViewPage({required this.postInfo});
}

class _StudyPostViewPageState extends State<StudyPostViewPage> {
  ...
  void _navigateToEditPage() async {
    final result =
    await Get.to(() => StudyPostEditPage(postInfo: widget.postInfo));
  ...
  /// 게시글 댓글 데베에 저장하는 함수
  Future<void> _saveCommentReport(
      TextEditingController controller, String id) async {
    ...
    if (reportReason.isNotEmpty) {
      String docId =
          FirebaseFirestore.instance.collection('commentReport').doc().id;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: GestureDetector(
          child:
          SvgPicture.asset('assets/icon/leftArrow.svg', fit: BoxFit.none), // X 아이콘
          onTap: () {
            var res;
            if (reload) res = true;
            Get.back(result: res);
                ...
                return <PopupMenuEntry>[
                  if (isPostAuthor) ...[
                    PopupMenuItem(
                      ...
                      child: Center(
                        child: Text('수정하기', style: f12bw500),
                      ),
                    ),
                    PopupMenuDivider(),
                    PopupMenuItem(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (widget.postInfo['code'] != '값이없음' ||
                          widget.postInfo['code'].length > 0)
                        Container(
                          child: MarkdownBody(
