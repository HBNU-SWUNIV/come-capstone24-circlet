  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          schedule['scheduleWriteDocId']==us.userList[0]['docId']?
          GestureDetector(
            onTap: (){
              showComponentDialog(context,'일정을 삭제하시겠습니까?', () async {
                await deleteSchedule(schedule['docId']);
                print('삭제하고 다시 가져오기${scs.schedule.value}');
                Get.back();
                Get.back();
              });

            },
            child: SvgPicture.asset('assets/icon/trash_can.svg'),
          ):Container(),
        ],
      ...
      GestureDetector(
        onTap: () async {
          ...
          if(check == false&&schedule['personList'].length<int.parse(schedule['maxPerson'])){
            await applySchedule(schedule['docId']);
            ...
            await cancelApplySchedule(schedule['docId']);
