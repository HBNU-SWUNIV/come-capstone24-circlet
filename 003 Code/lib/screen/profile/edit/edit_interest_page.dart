...
class EditInterestPage extends StatefulWidget {
  ...
  const EditInterestPage({Key? key, required this.itemList}) : super(key: key);
...
class _EditInterestPageState extends State<EditInterestPage> {
  ...
  void initState() {
    ...
    for(String item in widget.itemList){
      int index = iconNameList.indexOf(item);
      if (index != -1) {
      ...
      bottomSheet: GestureDetector(
        onTap: (){
          if(clickList.any((element) => element == true)){
              ...
              us.userDetailList[0]['interest'] = filteredList;
              print('?? ${us.userDetailList[0]['interest']}');
              us.userDetailList.refresh();
              Get.back();
          }
          setState(() {});
        },
